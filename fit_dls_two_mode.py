import numpy as np
import sys, os, glob, lmfit, pickle, DLSFunctions
...
def calc_g2(p, tau):
        ...
        A1 = p["A1"].value
        return DLSFunctions.calc_g2_twog(tau, A1, Gamma1, Gamma2, 1., f)
...
def calc_g2_single(p, tau):
        f = p["f"].value
        Gamma = p["Gamma"].value
        return DLSFunctions.calc_g2(tau, Gamma, 1., f)
...
class DLSViewer(QtGui.QMainWindow):
        def __init__(self, data, datlist, data_folder_path, parent = None):
                ...
                self.bvalid = QtGui.QPushButton('Toggle Valid')
                self.bsave = QtGui.QPushButton('Save Data')
                self.bswitchGamma = QtGui.QPushButton('Switch Gamma')
                
                self.lgamma1 = QtGui.QLabel("Gamma1: ")
                self.tgamma1 = QtGui.QLineEdit()
                
                self.lgamma2 = QtGui.QLabel("Gamma2: ")
                self.tgamma2 = QtGui.QLineEdit()
                
                self.lA1 = QtGui.QLabel("A1: ")
                self.tA1 = QtGui.QLineEdit()
                
                self.bplot = QtGui.QPushButton('Replot')
                self.brefit = QtGui.QPushButton('Refit')
                self.bsinglemodefit = QtGui.QPushButton('Single Mode Fit')
